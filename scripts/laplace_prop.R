...
log_h_grad <- function(x, mu, sigma_2) {
  ...
  return(zeta(1, x) - (x - mu)/sigma_2)
}

approx_ti <- function(mu, sigma_2) {
  # Approximate tilted inference for probit regression using Laplace's method
...
laplace_prop <- function(X, y, mu_beta, Sigma_beta,
                         r_init, Q_init,
                         min_pass, max_pass, thresh, verbose) {
  # Laplace propagation for probit regression
  N <- nrow(X)
  p <- ncol(X)
  Z <- X*(2*y - 1)
  ...
  for (pass in 1:max_pass) {
    ...
    for (n in sample(1:(N + 1))) {
      ...
      if (n < N + 1) {
        ...
        approx_ti_res <- approx_ti(mu_c_star, Sigma_c_star)
        Q_star_tilde <- approx_ti_res$Q
        r_star_tilde <- approx_ti_res$r
